import {
  ...
  QueryInput,
  QueryResponse,
  RequestOptions, UdfFieldInfo,
  UdfInfoResponse,
  UpdateResponse
} from "./index";

type Company = { id: number, companyName: string, userDefinedFields: [{ name: 'Test', value: string }] }
...
describe('client', () => {
  it('should have entities as properties', () => {
    ...
    expectTypeOf(AutotaskRestApi).instance.toHaveProperty('ConfigurationItemAttachments')
  })
})
...
describe('filter operators', () => {
  ...
  describe('equality', () => {
    it('should allow valid properties', () => {
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.eq, value: 0}]})
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.noteq, value: 0}]})
    })

    it('should disallow invalid properties', () => {
      // @ts-expect-error - `items` is not a valid property in this context.
      assertType<CountInput>({filter: [{op: FilterOperators.eq, items: []}]})
      // @ts-expect-error - `items` is not a valid property in this context.
      assertType<CountInput>({filter: [{op: FilterOperators.noteq, items: []}]})
    })
  })

  describe('comparison', () => {
    it('should allow valid properties', () => {
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.gt, value: 0}]})
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.gte, value: 0}]})
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.lt, value: 0}]})
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.lte, value: 0}]})
    })

    it('should disallow invalid properties', () => {
      // @ts-expect-error - `items` is not a valid property in this context.
      assertType<CountInput>({filter: [{op: FilterOperators.gt, items: []}]})
      // @ts-expect-error - `items` is not a valid property in this context.
      assertType<CountInput>({filter: [{op: FilterOperators.gte, items: []}]})
      // @ts-expect-error - `items` is not a valid property in this context.
      assertType<CountInput>({filter: [{op: FilterOperators.lt, items: []}]})
      // @ts-expect-error - `items` is not a valid property in this context.
      assertType<CountInput>({filter: [{op: FilterOperators.lte, items: []}]})
    })

    it('should disallow boolean values', () => {
      // @ts-expect-error - a boolean is not a valid value in this context.
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.gt, value: true}]})
      // @ts-expect-error - a boolean is not a valid value in this context.
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.gte, value: true}]})
      // @ts-expect-error - a boolean is not a valid value in this context.
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.lt, value: true}]})
      // @ts-expect-error - a boolean is not a valid value in this context.
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.lte, value: true}]})
    })

    it('should disallow null values', () => {
      // @ts-expect-error - `null` is not a valid value in this context.
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.gt, value: null}]})
      // @ts-expect-error - `null` is not a valid value in this context.
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.gte, value: null}]})
      // @ts-expect-error - `null` is not a valid value in this context.
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.lt, value: null}]})
      // @ts-expect-error - `null` is not a valid value in this context.
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.lte, value: null}]})
    })
  });

  describe('string', () => {
    it('should allow valid properties', () => {
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.contains, value: 'test'}]})
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.beginsWith, value: 'test'}]})
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.endsWith, value: 'test'}]})
    })

    it('should disallow invalid properties', () => {
      // @ts-expect-error - `items` is not a valid property in this context.
      assertType<CountInput>({filter: [{op: FilterOperators.contains, items: []}]})
      // @ts-expect-error - `items` is not a valid property in this context.
      assertType<CountInput>({filter: [{op: FilterOperators.beginsWith, items: []}]})
      // @ts-expect-error - `items` is not a valid property in this context.
      assertType<CountInput>({filter: [{op: FilterOperators.endsWith, items: []}]})
    })

    it('should disallow number values', () => {
      // @ts-expect-error - a number is not a valid value in this context.
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.contains, value: 0}]})
      // @ts-expect-error - a number is not a valid value in this context.
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.beginsWith, value: 0}]})
      // @ts-expect-error - a number is not a valid value in this context.
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.endsWith, value: 0}]})
    })

    it('should disallow null values', () => {
      // @ts-expect-error - `null` is not a valid value in this context.
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.contains, value: null}]})
      // @ts-expect-error - `null` is not a valid value in this context.
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.beginsWith, value: null}]})
      // @ts-expect-error - `null` is not a valid value in this context.
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.endsWith, value: null}]})
    })

    it('should disallow boolean values', () => {
      // @ts-expect-error - a boolean is not a valid value in this context.
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.contains, value: true}]})
      // @ts-expect-error - a boolean is not a valid value in this context.
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.beginsWith, value: true}]})
      // @ts-expect-error - a boolean is not a valid value in this context.
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.endsWith, value: true}]})
    })
  })

  describe('existence', () => {
    it('should allow valid properties', () => {
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.exist}]})
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.notExist}]})
    })

    it('should disallow invalid properties', () => {
      // @ts-expect-error - `value` is not a valid property in this context.
      assertType<CountInput>({filter: [{op: FilterOperators.exists, value: 'test'}]})
      // @ts-expect-error - `value` is not a valid property in this context.
      assertType<CountInput>({filter: [{op: FilterOperators.notexists, value: 'test'}]})
    })
  })

  describe('set', () => {
    it('should allow valid properties', () => {
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.in, value: [0]}]})
      assertType<CountInput>({filter: [{field: 'id', op: FilterOperators.notIn, value: [0]}]})
    })

    it('should disallow invalid properties', () => {
      // @ts-expect-error - `items` is not a valid property in this context.
      assertType<CountInput>({filter: [{op: FilterOperators.in, items: []}]})
      // @ts-expect-error - `items` is not a valid property in this context.
      assertType<CountInput>({filter: [{op: FilterOperators.notIn, items: []}]})
    })
...
describe('query input', () => {
  it('should allow any IncludeFields for untyped queries', () => {
    assertType<QueryInput>({filter: [], IncludeFields: ['id', 'companyName', 'Test']})
  })

  it('should allow valid IncludeFields for typed queries', () => {
    assertType<QueryInput<Company>>({filter: [], IncludeFields: ['id', 'companyName', 'Test']})
  })

  it('should not allow invalid IncludeFields for typed queries', () => {
    // @ts-expect-error - `invalid` value is not allowed
    assertType<QueryInput<Company>>({filter: [], IncludeFields: ['id', 'companyName', 'Test', 'invalid']})
  })
})

describe('field info', () => {
  type NonStringDataType = "boolean" | "integer" | "long" | "double" | "decimal" | "datetime"
  ...
  describe('picklists', () => {
    ...
    describe('standard picklists', () => {
      type StandardPicklist = Extract<Picklist, {picklistParentValueField: ''}>

      it('should have an empty string parentValue in picklistValues', () => {
        expectTypeOf<StandardPicklist>().toMatchTypeOf<{ picklistValues: {parentValue: ''}[] }>()
      })
    })
    ...
    describe('child picklists', () => {
      ...
      it('should have string picklistParentValueField', () => {
        expectTypeOf<ChildPicklist>().toMatchTypeOf<{ picklistParentValueField: string }>()
      })

      it('should have a string parentValue in picklistValues', () => {
        expectTypeOf<ChildPicklist>().toMatchTypeOf<{ picklistValues: {parentValue: string}[] }>()
      })

      it('should not have have an empty string parentValue in picklistValues', () => {
        // @ts-expect-error - `parentValue` should be a string.
        expectTypeOf<ChildPicklist>().toMatchTypeOf<{ picklistValues: {parentValue: ''}[] }>()
      })
    })
  ...
  describe('references', () => {
    ...
    it('should have an empty string picklistParentValueField', () => {
      expectTypeOf<Reference>().toMatchTypeOf<{ picklistParentValueField: '' }>()
    })
...
describe('user-defined field info', () => {
  ...
  describe('picklists', () => {
    ...
    describe('standard picklists', () => {
      type StandardPicklist = Extract<Picklist, {picklistParentValueField: null}>
  ...
  describe('references', () => {
    ...
    it('should have a null picklistParentValueField', () => {
      expectTypeOf<Reference>().toMatchTypeOf<{ picklistParentValueField: null }>()
    })
...
describe('entity methods', () => {
  describe('query', () => {
    it('should return query response for untyped requests', () => {
      expectTypeOf(api.Companies.query).returns.resolves.toEqualTypeOf<QueryResponse>()
    })

    it('should return typed query response for typed requests', () => {
      expectTypeOf(api.Companies.query<Company>).returns.resolves.toEqualTypeOf<QueryResponse<Company>>()
    })
  ...
  describe('count', () => {
    it('should match the expected function type', () => {
      assertType<(query: CountInput) => Promise<CountResponse>>(api.Companies.count)
    })
  })

  describe('get', () => {
    it('should return get response for untyped requests', () => {
      expectTypeOf(api.Companies.get).returns.resolves.toEqualTypeOf<GetResponse>()
    })
    ...
    it('should require two parameters for attachment entities', () => {
      expectTypeOf(api.ConfigurationItemAttachments.get).parameters.toEqualTypeOf<[number, number]>()
    })
  ...
  describe('update', () => {
    it('should return update response for requests', () => {
      expectTypeOf(api.Companies.update).returns.resolves.toEqualTypeOf<UpdateResponse>()
    })
    ...
    it('should only require one parameter for normal entities', () => {
      assertType<(
        toSave: Entity,
        opts?: RequestOptions
      ) => Promise<UpdateResponse>>(api.Companies.update)
    })
    ...
    it('should require two parameters for child entities', () => {
      assertType<(
        ...
        toSave: Entity,
        opts?: RequestOptions
      ) => Promise<UpdateResponse>>(api.ConfigurationItemAttachments.update)
    })
  })
  ...
  describe('create', () => {
    it('should only require one parameter for normal entities', () => {
      assertType<(
        toSave: Entity,
        opts?: RequestOptions
      ) => Promise<CreateResponse>>(api.Companies.create)
    })
    ...
    it('should require two parameters for child entities', () => {
      assertType<(
        ...
        toSave: Entity,
        opts?: RequestOptions
      ) => Promise<CreateResponse>>(api.ConfigurationItemAttachments.create)
    })
  })
  ...
  describe('delete', () => {
    ...
    it('should require two parameters for child entities', () => {
      assertType<(
        parentId: number,
        id: number
      ) => Promise<DeleteResponse>>(api.ConfigurationItemAttachments.delete)
    })
  })
  ...
  describe('replace', () => {
    it('should only require two parameters for normal entities', () => {
      assertType<(
        toSave: Entity,
        opts?: RequestOptions
      ) => Promise<UpdateResponse>>(api.Companies.replace)
    })
    ...
    it('should require three parameters for child entities', () => {
      assertType<(
        ...
        toSave: Entity,
        opts?: RequestOptions
      ) => Promise<UpdateResponse>>(api.ConfigurationItemAttachments.replace)
    })
  })
